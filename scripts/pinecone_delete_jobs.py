...
def main():
    ...
    res = openai.Embedding.create(input=job_text, engine=MODEL)
    embed = res["data"][0]["embedding"]

    query_response = index.query(vector=embed, top_k=200, include_metadata=True)
    metadata = query_response["matches"]

    # Find the items with content field starting with ```json
    ids_to_delete = []
    print(metadata)
    for item in metadata:
        item_id = item["id"]
        item_metadata = item["metadata"]
        if item_metadata["content"].startswith("```json"):
            ids_to_delete.append(item_id)

    # Delete the identified items
    print(f"ids_to_delete counts: {len(ids_to_delete)}")
    if ids_to_delete:
